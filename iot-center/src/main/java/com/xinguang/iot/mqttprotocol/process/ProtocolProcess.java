package com.xinguang.iot.mqttprotocol.process;
...
import com.xinguang.iot.mqttprotocol.MQTTMesageFactory;
import com.xinguang.iot.mqttprotocol.message.*;
import com.xinguang.iot.mqttprotocol.process.Impl.IdentityAuthenticator;
import com.xinguang.iot.mqttprotocol.process.Impl.dataHandler.MapDBPersistentStore;
import com.xinguang.iot.mqttprotocol.process.Interface.ISessionStore;
import com.xinguang.iot.mqttprotocol.process.event.job.RePublishJob;
import com.xinguang.iot.mqttprotocol.process.subscribe.SubscribeStore;
import com.xinguang.iot.mqttprotocol.process.subscribe.Subscription;
import com.xinguang.iot.mqttprotocol.process.event.PubRelEvent;
import com.xinguang.iot.mqttprotocol.process.event.PublishEvent;
import com.xinguang.iot.mqttprotocol.process.event.job.RePubRelJob;
...
import com.xinguang.iot.util.QuartzManager;
import com.xinguang.iot.util.StringTool;
...
import com.xinguang.iot.mqttprotocol.message.ConnAckMessage.ConnectionStatus;
import com.xinguang.iot.mqttprotocol.process.Interface.IAuthenticator;
import com.xinguang.iot.mqttprotocol.process.Interface.IMessagesStore;
...
//		this.sessionStore = storge;
	...
	//将此类单例
	private static volatile ProtocolProcess INSTANCE;
	public static ProtocolProcess getInstance(){
		if (INSTANCE == null) {
			synchronized(ProtocolProcess.class){
				if(INSTANCE == null){
	...
	public void processConnect(Channel client, ConnectMessage connectMessage){
		Log.info("处理ClientID:{"+connectMessage.getPayload().getClientId()+"}的Connect的数据");
		...
		//----------------------------------------------------------------------------------------------------------
		//处理身份验证（userNameFlag和passwordFlag,username为企业注册时的账号，password为平台分配的apiKey）
		if (connectMessage.getVariableHeader().isHasUsername() &&
				connectMessage.getVariableHeader().isHasPassword()) {
			String loginName = connectMessage.getPayload().getUsername();
			String apiKey = connectMessage.getPayload().getPassword();
			//此处对企业进行鉴权
			if (!authenticator.checkValid(loginName, apiKey)) {
				Log.info("企业鉴权未通过，不接收此客户端,客户端ID：{"+connectMessage.getPayload().getClientId()+"}");
				ConnAckMessage connAckMessage = (ConnAckMessage) MQTTMesageFactory.newMessage(
						FixedHeader.getConnAckFixedHeader(),
			...
			Log.info("没有鉴权信息，无法进行鉴权，不接收此客户端,客户端ID：{"+connectMessage.getPayload().getClientId()+"}");
			ConnAckMessage connAckMessage = (ConnAckMessage) MQTTMesageFactory.newMessage(
					FixedHeader.getConnAckFixedHeader(),
		...
		if (connectMessage.getPayload().getClientId() == null || connectMessage.getPayload().getClientId().length() == 0) {
			...
			if (connectMessage.getVariableHeader().isCleanSession()) {
				...
				Log.info("客户端ID为空，cleanSession为0，根据协议，不接收此客户端，客户端ID：{"+connectMessage.getPayload().getClientId()+"}");
				ConnAckMessage connAckMessage = (ConnAckMessage) MQTTMesageFactory.newMessage(
						FixedHeader.getConnAckFixedHeader(),
						new ConnAckVariableHeader(ConnectionStatus.IDENTIFIER_REJECTED, false),
						null);
			...
			//进行设备认证
			if(!authenticator.checkDevice(connectMessage.getPayload().getClientId())){
				Log.info("客户端ID:{"+connectMessage.getPayload().getClientId()+"}未在平台进行注册绑定，不接收此客户端");
				ConnAckMessage connAckMessage = (ConnAckMessage) MQTTMesageFactory.newMessage(
						FixedHeader.getConnAckFixedHeader(),
						new ConnAckVariableHeader(ConnectionStatus.IDENTIFIER_REJECTED, false),
						null);
...
//		}
		...
		NettyAttrManager.setAttrKeepAlive(client, keepAlive*2);
		//添加心跳机制处理的Handler
		client.pipeline().addFirst("idleStateHandler", new IdleStateHandler(keepAlive*2, Integer.MAX_VALUE, Integer.MAX_VALUE, TimeUnit.SECONDS));
...
//		}
	...
	public void processPublic(Channel client, PublishMessage publishMessage){
		...
		Log.info("处理ClientID:{"+clientID+"}的publish的数据");
	...
	public void processPublic(Channel client, WillMessage willMessage){
		String clientID = NettyAttrManager.getAttrClientId(client);
		Log.info("处理ClientID:{"+clientID+"}的遗言的publish数据");
...
//		int sendPackageID = PackageIDManager.getNextMessageId();
	...
	public void processPingReq(Channel client, Message pingReqMessage){
		Log.info("收到clientID{"+ NettyAttrManager.getAttrClientId(client)+"}的心跳包");
	...
	public void processDisconnet(Channel client, Message disconnectMessage){
		String clientID = NettyAttrManager.getAttrClientId(client);
		Log.info("客户端:{"+clientID+"}断开链接,Disconnet");
		boolean cleanSession = NettyAttrManager.getAttrCleanSession(client);
		if (cleanSession) {
...
//	    messagesStore.removeQosPublishMessage(pubRelKey);
	...
	private void processMessage(String topic, ByteBuf message) {
		byte[] req = new byte[message.readableBytes()];
		message.readBytes(req);
		try {
			String body = new String(req,"UTF-8");//传过来的数据为设备id，传感器id，数值
			String[] bodys = body.split(",");
			System.out.println(topic+":"+body+"，接收时间为："+new Date());
			DataService dataService =  SpringContextHolder.getBean(DataService.class);
			SensorService sensorService = SpringContextHolder.getBean(SensorService.class);
